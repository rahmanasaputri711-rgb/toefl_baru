...
function updatePreview(val) {
    ...
    el.style.color = answers.length > 0 ? '#4ade80' : '#34d399';
...
function simpan() {
    ...
    if (!blanks) return showAlert('Tidak ada blank [...] ditemukan. Gunakan format [kata].','danger');
    // Tidak perlu validasi jumlah — sistem otomatis sesuaikan rentang nomor soal
    ...
    fetch(`/admin/paket-builder/modul/${MODUL_ID}/missing-letters`,{
        ...
        body: JSON.stringify({fill_text:val, tingkat_kesulitan:'medium'}),
    })
    .then(r => {
    ...
    .then(d=>{
        if (d.ok) {
            showAlert(d.msg + ' Mengalihkan...','success');
            setTimeout(() => {
                window.location.href = d.redirect || `/admin/paket-builder/paket/${MODUL_ID}`;
            }, 1000);
        } else {
